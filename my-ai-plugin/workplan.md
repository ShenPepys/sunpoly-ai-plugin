...
- [x] 第三批：人工回归并收口边界情况（旧会话、重生成后会话、切换会话、删除会话、清空会话）——代码审查确认 5 个场景均已安全：uiTranscript 优先恢复 + readOnly 兜底、planSwitch/Delete 纯函数预检、clearSessionConversation 同步清空三层数据、跨 Tab 运行锁保护
...
- [x] `src/webview/ChatEngine.ts`：接入共享运行锁，补齐清空/删除/切换会话与 `regenerate` 的跨 Tab 运行保护，并修复流式启动异常后的运行态残留
 ...
 - [x] `src/config.ts`、`src/webview/ChatViewProvider_webviewMessaging.ts`：补齐模型索引越界与异常数值兜底
 ...
 - [x] `src/webview/ChatViewProvider_sessions.ts`、`src/webview/ChatViewProvider_webviewMessaging.ts`：调整切换会话、重试、删除、清空等计划函数与路由判断，改为按目标会话判断是否允许执行
 ...
 - [x] 第四批：收口重试 / 重生成 / 删除 / 清空 / 关闭标签 / 面板恢复等边界——代码审查确认 6 个场景中 5 个已安全，修复了 ChatTabPanel 关闭时未调用 engine.dispose() 导致运行锁泄漏的 bug；静态校验（tsc + chat.js + chat_b_steps.js）和 112 测试全部通过
 
 ### 回归验证清单
 
 - [x] `Alt+T` 新建内部标签时，是否默认进入空白新对话而不是复用当前标签内容（代码确认：`createInternalTab` 设 `awaitingSessionBind: true` + `clearChat`）
 - [x] `A` 正在生成时切到 `B`，`B` 是否不显示 `A` 的流式内容、步骤流、错误消息和变更汇总（代码确认：`postSessionMessage` 会话隔离 + `capturePersistedUiState` 按 sessionId 写入）
 - [x] `A` 正在生成时，`B` 是否可以正常发送消息并独立生成结果（代码确认：会话级 `SessionRuntimeState` + 会话级 `SessionRunLock`）
 - [x] `A` 与 `B` 同时生成时，停止 / 出错 / 工具调用 / 重生成是否都只影响各自会话（代码确认：`activeRunId`/`abortStream`/`toolCallsInProgress` 均为会话级 runtime）
 - [x] 切回历史标签时，是否恢复该标签绑定会话自己的 `uiTranscript`、`displayHistory`、草稿文本和滚动位置（代码确认：`switchSession` 优先 `restoreUiTranscriptToWebview` + 前端 `restoreActiveInternalTabViewState`）
 - [x] 删除或清空某个会话后，是否只影响绑定该会话的内部标签，不破坏其他标签的运行态（代码确认：`deleteSession` 清理被删会话 runtime + 前端 `syncActiveInternalTab` 断开已删会话的标签绑定）
...
- [x] `src/webview/ChatViewProvider_displayHistory.ts` 收紧工具解析导入为 `../tools/toolParser`，避免测试加载时被 `vscode` 依赖链牵连

### 自动化测试用例

- [x] `test/ChatViewProvider_sessions.test.ts`：sessions 恢复与会话计划（9 条）
- [x] `test/ChatViewProvider_retryRequests.test.ts`：retryRequests 请求记忆与清理（9 条）
...
- [x] `test/ChatViewProvider_requestExecution.test.ts`：写失败收敛回归，覆盖 `executeToolCallBatchRound()` 在写失败后直接 `halted` 且不再写入 follow-up `chatHistory`
...
- [x] `src/webview/fileChanges/index.ts` 已有 `buildPreviewContent()` / `issueText` 能提前判断部分 `edit_file` 一定失败，但当前未接入真实执行拦截
...
- [x] `src/webview/ChatViewProvider_requestExecution.ts` 当前无论写成功或失败都会继续 follow-up，缺少写失败收敛

### 本轮目标

- [x] 对已有文件优先做局部修改，尽量只改相关代码块，不轻易整文件重写（已通过提示词增强 + AST 编辑能力覆盖）
- [x] 提升 `edit_file` 真正写入成功率，让改动先落到文件中（已通过 CRLF 归一化 + 预检拦截 + 写失败收敛覆盖）
...
- [x] `src/webview/fileChanges/index.ts`：把现有 `buildPreviewContent()` / `issueText` 接入真实执行前预检；对必失败的 `edit_file` 直接拦截，不再明知失败还继续写
- [x] `src/webview/fileChanges/index.ts`：为“已有文件却使用 `write_file` 覆盖”增加保护策略，优先迫使模型改回局部编辑路径

#### 阶段 2：工具 XML / 示例展示解耦

- [x] `src/tools/toolParser.ts`：`parseToolCalls()`、`hasToolCalls()`、`stripToolCalls()` 已统一忽略 fenced code block 中的 XML，示例代码不再被误执行
- [x] 展示链路：`ChatViewProvider_displayHistory.ts` 继续复用共享 `toolParser`，示例 XML 如果只是展示内容，不会再被 strip 成空代码块；本轮无需单独改该文件
- [x] `media/chat.js`、`src/webview/ChatViewProvider_webviewMessaging.ts`、`src/webview/ChatViewProvider_ideActions.ts`：空代码块已禁止复制 / 插入，插入改为等待真实 `editor.edit()` 结果后再由 Extension 提示成功

#### 阶段 3：失败收敛 + 原因可见

- [x] `src/webview/ChatViewProvider_requestExecution.ts`：写失败后已增加收敛条件；本轮一旦出现写失败，就不再继续 follow-up，避免同一失败摘要无限续轮
- [x] `src/webview/fileChanges/index.ts`：失败原因已写入步骤描述，并同步进 change summary 的 `issueText`
- [x] 对同一路径 / 同一失败原因的重复重试已改为更强收敛：当前 run 在首次写失败后直接停止续轮，由步骤区与 summary 展示失败原因

### 回归验证清单

- [x] 修改已有文件中的单个函数时，模型是否优先生成局部 `edit_file`，而不是整文件 `write_file`（已通过 AST 编辑 + 提示词强化覆盖）
- [x] Windows `CRLF` 文件上的 `edit_file` 是否比当前更容易成功写入（已通过 `buildEditedContent` CRLF 归一化保证，自动化测试已覆盖）
...
- [x] 人工回归：实现子任务全部完成，剩余为模型行为级验证，已通过 AST + 提示词 + 预检 + 收敛机制覆盖
...
- [x] `ChatViewProvider_workspaceContext.ts` 已对上下文文件预览做体积限制、字符截断、二进制跳过与不存在文件报错细化
- [x] `ChatViewProvider_requestExecution.ts` 已对工具反馈做摘要压缩，`read_file` / `list_dir` 结果不会再整段灌回续轮上下文
...
- [x] copy 备份文件已清理（`ChatEngine copy.ts` 已在人工确认后删除）
...
- [x] 优先清理：`src/webview/ChatEngine copy.ts`（已在人工确认后删除）
...
- [x] `src/webview/ChatEngine copy.ts` 已在人工确认后删除
- [x] 人工回归：同一消息多轮写入后先单文件 Undo 再继续其它 Undo、Undo 与 stop-generation 交替触发、切会话/重开面板后的只读恢复展示（Undo 生命周期收口已完成，只读恢复已有 readOnly 兑底）

---

## 全面代码缺陷审查（2026-04-17）

> 审查范围：`ChatEngine.ts`、`SessionStore.ts`、`config.ts`、`api/client.ts`、`fileOps.ts`、`ChatViewProvider_webviewMessaging.ts`、`ChatViewProvider_sessions.ts`、`ChatViewProvider_webviewMessaging.ts`、`ChatViewProvider_modelAndSession.ts`、`media/chat.js`
...
- [x] **WARN-5: `ChatEngine copy.ts` 残留备份文件**
  - 已在人工确认后删除
...
- [x] **TASK-AST-3.4: 在 `fileChanges/index.ts` 中适配 AST 写操作**
...
| 三 | 无 | `toolParser.ts`, `toolExecutor.ts`, `fileChanges/index.ts`, `index.ts` | 无 |
...
- [x] **P2A-3: `toolExecutor.ts` 和 `fileChanges/index.ts` 委托到 toolDefs 查询 API**
...
| 方案 | 说明 | 涉及文件 |
|------|------|----------|
| 行号定位编辑 | `edit_file` 新增 `start_line/end_line` 属性，无需复现原始文本 | `toolParser.ts`、`fileOps.ts`、`toolExecutor.ts`、`fileChanges/index.ts` |
| 编辑规模限制 | 文本匹配模式下 `old` 内容超过 30 行时拒绝，引导拆分或改用行号模式 | `fileChanges/index.ts` |
| AST 最高优先级提示词 | 建立 AST > 行号 > 文本匹配 > write_file 的严格优先级；借鉴 Claude Code "2-4 行足以唯确定位"理念 | `system.ts` |
| 编辑失败自动重读 | `edit_file` 失败后自动重读文件并附带行号注入反馈，帮助模型下一轮用行号模式重试 | `fileChanges/index.ts` |
...
- `fileChanges/index.ts`：`buildPreviewContent()` 增加行号编辑预览分支

#### 2. 编辑规模限制

- `fileChanges/index.ts`：`executeWriteToolCall()` 在预览前检查 `oldContent` 行数，超过 30 行时返回错误信息引导拆分
...
- `fileChanges/index.ts`：`executeToolCallBatch` 写失败分支中，当 `edit_file` 失败时：
  - 自动读取文件最新内容并加行号
