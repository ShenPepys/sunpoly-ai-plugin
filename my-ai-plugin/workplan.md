...
- [ ] 把工具 XML 示例与真实执行彻底分离，避免“展示示例被当真执行”
- [ ] 写失败后及时收敛，并把真实原因展示给用户
...
- [ ] `src/tools/toolParser.ts`：让 `parseToolCalls()`、`hasToolCalls()`、`stripToolCalls()` 忽略 fenced code block 中的 XML，避免示例代码被误执行
- [ ] `src/webview/ChatViewProvider_c_displayHistory.ts`：保证示例 XML 如果只是展示内容，不会被 strip 成空代码块
- [ ] `media/chat.js`、`src/webview/ChatViewProvider_l_webviewDispatch.ts`、`src/webview/ChatViewProvider_j_ideActions.ts`：空代码块禁止复制 / 插入，插入必须等待真实结果后再提示成功

#### 阶段 3：失败收敛 + 原因可见

- [ ] `src/webview/ChatViewProvider_p_requestExecution.ts`：写失败后增加收敛条件，避免同一失败摘要无限续轮
- [ ] `src/webview/ChatViewProvider_d_fileChanges.ts`：把失败原因写入步骤描述，必要时同步进 change summary 的 `issueText`
- [ ] 复查同一路径、同一 `old/new`、同一失败原因的重复重试，必要时直接停止并提示用户

### 回归验证清单

- [ ] 修改已有文件中的单个函数时，模型是否优先生成局部 `edit_file`，而不是整文件 `write_file`
- [ ] Windows `CRLF` 文件上的 `edit_file` 是否比当前更容易成功写入
- [ ] fenced code block 中的 XML 示例是否不再被执行，也不会被剥空
- [ ] 代码块为空时，复制 / 插入是否不再假成功
- [ ] 写失败后是否会停止重复读写，而不是继续续轮到高轮次
- [ ] 步骤区 / 汇总区是否能看到真实失败原因
