...
```
src/
├── extension.ts          # 入口：命令注册、状态栏、生命周期
├── config.ts             # 配置管理（多模型支持）
├── logger.ts             # 日志工具
├── api/
│   ├── types.ts          # API 类型定义
│   └── client.ts         # OpenAI 兼容 HTTP 客户端（流式 SSE）
├── commands/
│   └── handler.ts        # 5 种命令处理器
├── prompts/
│   ├── types.ts          # Prompt 类型定义
│   ├── system.ts         # 系统提示词（分层设计）
│   ├── explain.ts        # 解释代码 Prompt
│   ├── fix.ts            # 修复代码 Prompt
│   ├── optimize.ts       # 优化代码 Prompt
│   ├── complete.ts       # 续写代码 Prompt
│   ├── test.ts           # 生成单测 Prompt
│   └── chat.ts           # 通用聊天 Prompt
├── tools/                # 工具调用系统
├── utils/
│   ├── editor.ts         # 编辑器上下文
│   └── context.ts        # 环境上下文 + 项目类型检测
└── webview/
    ├── ChatViewProvider.ts  # Webview 核心逻辑
    └── messageTypes.ts      # 消息类型定义
media/
├── chat.js               # 前端主逻辑
├── chat_a_render.js      # Markdown 渲染模块
└── chat.css              # 样式
```
...
- **API 协议**：OpenAI 兼容（HTTP/HTTPS + SSE 流式）
- **前端**：原生 DOM + CSS（Webview 沙箱环境）
